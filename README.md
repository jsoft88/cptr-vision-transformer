...
I am adding code for the argument_parser module to clearly shows the arguments that need to be provided and their default value.
...
self.general_group.add_argument(
    ...
    help=f'file system to use when saving model state. One of: local-fs, s3-fs'
)
...
self.general_group.add_argument(
    ...
    help=f'Type of model initializer to use. One of: vit_base_16_384_bert',
    default=None
)
...
self.general_group.add_argument(
    ...
    help=f'Type of input for the current execution mode. One of: batch-dm, api-dm, local-batch-dm',
    default=None
)
...
For the second configuration type, you can use a yaml file. The format of the file is customized to the cptr model and there are no rules regarding what the content of the file should be, however, make sure that there is a class tightly coupled to the content of the file (in the case of cptr, this class is `cptr_model.config.specifics.cptr.architecture_config_file_manager.ArchitectureConfigFileManager`). This file is later added to the CLI params. Below is an excerpt of what this config.yaml might look like:
...
Notice that we define specific blocks, types, dimensions, and so on for the cptr model. Thanks to [hydra](https://hydra.cc/docs/intro/), we can override different entries in the config yaml directly from CLI. For example, besides adding all the required parameters when executing the model, we can also add the field we want to modify in our yaml file: model.cptr.architecture.blocks.encoder.layers.input.sublayers.patch_embedding.params.dim=512